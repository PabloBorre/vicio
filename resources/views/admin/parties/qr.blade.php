...
<body class="min-h-screen bg-zinc-950 flex flex-col items-center justify-center px-6 py-10 text-white">
    ...
    <div class="w-full max-w-sm bg-zinc-900 border border-zinc-800 rounded-2xl px-4 py-3 flex items-center gap-3 mb-6">
        ...
        <button
            onclick="navigator.clipboard.writeText('{{ $registerUrl }}').then(() => { this.innerText = '✓'; setTimeout(() => this.innerText = 'Copiar', 1500) })"
        ...
        @auth
            @if(auth()->user()->is_admin)
                <a
                    ...
                    class="w-full text-center text-zinc-500 text-sm py-2 hover:text-zinc-300 transition-colors"
                >
                    ← Volver al panel
                </a>
            @endif
